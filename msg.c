	                    ...
	                    int check_rc)
{
	...
	if (!val) {
		applog(LOG_ERR, "submit_work_aux json_rpc_call (getauxblock) failed");
		goto out;
	}
			...
			CURL *curl, const char *hexstr, const char **reason)
{
	...
	*reason = NULL;
	...
	if(work)
	{
		...
		if (!val) {
			applog(LOG_ERR, "submit_work json_rpc_call (getworkex) failed");
			goto out;
		}
	} else {
		...
		if (!val) {
			applog(LOG_ERR, "submit_work json_rpc_call (getwork) failed");
			goto out;
		}
	}
